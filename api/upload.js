...
// Store bisa public atau private (dipilih saat membuat Blob store); coba keduanya.
  ...
  if (!dry) {
    ...
    for (const access of ACCESS) {
      try {
        ...
        errs.push(`${access}: ${e.message}`);
      }
    }
    if (errs.length)
      return Response.json({ error: "Gagal menyimpan ke Vercel Blob. " + errs.join(" | ") }, { status: 500 });
  }
